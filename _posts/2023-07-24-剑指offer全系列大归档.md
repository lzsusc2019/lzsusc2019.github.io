...
先判断是否有环，p、q分别从头，和从碰撞点(刚才1倍2倍相遇点)开始走，再次相遇的点是连接点
...
### [LeetCode 746 爬楼梯的最少成本](https://leetcode.cn/problems/min-cost-climbing-stairs/)
...
给定一个数组 `prices` ，其中 `prices[i]` 是一支股票第 i 天的价格。设计一个算法来计算你所能获取的最大利润。你可以多次买卖股票，但你必须在购买前卖出之前的股票。
...
输出: 7
解释: 在第 2 天买入（price=1），第 3 天卖出（price=5），利润=4。
     然后在第 4 天买入（price=3），第 5 天卖出（price=6），利润=3。
```
...
输入: prices = [1,2,3,4,5]
输出: 4
解释: 在第 1 天买入（price=1），第 5 天卖出（price=5），利润=4。
     注意你不能在买入前卖出股票。
```
...
**贪心算法**：因为股票买卖可以无限次，所以只要相邻两天价格上涨，就在前一天买入后一天卖出。
...
给定一个数组 `prices` ，其中 `prices[i]` 是一支股票第 i 天的价格。你不能同时参与多笔交易（必须在再次购买前卖出之前的股票）。设计一个算法来计算你所能获取的最大利润。最多可以完成两笔交易。
...
输出: 6
解释: 
- 在第 4 天（price=0）买入，第 6 天（price=3）卖出，利润=3
- 在第 7 天（price=1）买入，第 8 天（price=4）卖出，利润=3
- 总利润=6
```
...
输出: 4
解释: 
- 在第 1 天买入（price=1），第 5 天卖出（price=5），利润=4
- 只完成了一笔交易
```
...
给定一个整数 `k` 和一个数组 `prices` ，其中 `prices[i]` 是第 i 天的股票价格。你最多可以完成 `k` 笔交易。你不能在买入股票前卖出股票。设计一个算法来计算你能获得的最大利润。
...
输入: k = 2, prices = [2,4,6]
输出: 2
解释: 在第 1 天（price=2）买入，第 2 天（price=4）卖出，利润=2。
     或者第 2 天买入第 3 天卖出，但 k=2 所以最多两笔。
```
...
输出: 7
解释: 
- 第 2 天买入（price=2），第 3 天卖出（price=6），利润=4
- 第 4 天买入（price=5），第 5 天卖出（price=3），利润=-2（亏损）
- 第 5 天买入（price=0），第 6 天卖出（price=3），利润=3
- 总利润=4-2+3=5（实际应该选择最优的两笔：第2天买入第3天卖出+第5天买入第6天卖出=4+3=7）
```

#### 解析

动态规划扩展：最多 k 笔交易
- `dp[i][j]`：第 i 天完成 j 笔交易后，不持有股票的最大利润
- `dp[i][j+1]`：第 i 天完成 j 笔交易后，持有股票的最大利润
...
### [LeetCode 771 宝石与石头](https://leetcode.cn/problems/jewels-and-stones/)
